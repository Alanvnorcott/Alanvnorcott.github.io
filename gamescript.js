...
function resizeCanvas() {
    ...
    if (window.innerWidth < 500 || window.innerHeight < 500) {
        ...
        ctx.fillStyle = 'white';
        ctx.font = '20px Arial';
        ctx.fillText('Sorry, this game is not available on mobile devices.', 50, 50);
    }
}

// Call resizeCanvas() function on window resize event
window.addEventListener('resize', resizeCanvas);

// Add touch event listeners for mobile controls if the screen size is appropriate
if (window.innerWidth >= 500 && window.innerHeight >= 500) {
